...
{
        for (uint idxCascade = 0; idxCascade < cascade_responses.size(); ++idxCascade)
		{
			Mat respImageCascade = cascade_responses[idxCascade][0];

            for (uint idxDepth = 1; idxDepth < cascade_responses[idxCascade].size(); ++idxDepth)
				respImageCascade =
...
vector < Point3f > applyApproxFilters(const Mat & indatav, const TILDEobjects & tilde_obj,
				       ...
				       Mat * score)
{
    const float scaleKeypoint = 10.0;
...
// resize back
    ...
    for (uint i = 0; i < res_with_score.size(); ++i) {
						    ...
						    const vector < Mat > &convt_image)
{
    ...
    for (uint idxFilter = 0; idxFilter < cas.filters.size() / 2; idxFilter++) {
		//int idxOrig = 0;
...
TILDEobjects getTILDEApproxObjects(const string & name, void *_p)
{
	...
	while (getline(fic, lineread)) {
        ...
        std::cout<<"Filter row: "<<Mat(r)<<std::endl;
