...
void DescriptorAssembler::buildUp(){
    ...
    if(projectFase==BASE){
        ...
        module_size_list = {3, // 1 - Color channel
                            ...
                            //=============== Here comes the module activation section =============
//                            14,// Same as the module number
    ...
    std::vector<int> module_activator;
     ...
     * 10- Sobel or Sharr
*/
    int number_of_preproc_modules = 10;

    int module_iterator = 0;
    int position_counter = 0;
    int eval_point = 0;
    int activator_iterator = 0;
    ...
    for (uint t_it=0;t_it<descriptorGenome.size();++t_it){
        ...
        //fills the current genome string with bits
        ...
        if(t_it==(uint) eval_point){
            ...
            //!The first 10 module are converted to number
            if(module_iterator<number_of_preproc_modules){

                //Binary string to Parameter value
                int binValue = Matek::binVectorToInt(_active_elem);
                int moduleValue = Matek::binaryToDecimal(binValue);

                //Loads parameter to the corresponding Image processing or Machine Learning function
                loadValueToDescriptor(module_iterator,moduleValue);

                //One step forward to the next module
                _active_elem.clear();
                module_iterator++;
                eval_point+=module_size_list[module_iterator];
//            }else if(module_iterator==number_of_preproc_modules){

//            module_activator = _active_elem;
                ...
                if(areActiveElements(_active_elem)/*&&module_activator[activator_iterator]==1*/){
                    //Creating the module from the genome string, calculating its size, and loads into the descriptor module vector
                    ...
                    //The active element vector is cleaned for the next module
                    _active_elem.clear();
                    module_iterator++;
                    position_counter++;
                    activator_iterator++;
                    eval_point+=module_size_list[module_iterator];
                }else if (!areActiveElements(_active_elem)/* || module_activator[activator_iterator]==0*/){

                    int inact_module_real_size = sumOfVector(_active_elem);
                    DescriptorModule inactive_module(function_module_it,_active_elem,inact_module_real_size, 0,projectFase);
                    //Saves inactive modules for
   ...
   // assembleDescriptor->moduleActivator(module_activator);
...
bool DescriptorAssembler::areActiveElements(std::vector<int>& _active_elem){

    bool _areActiveElements = true;
    int sum_of_elems=sumOfVector(_active_elem);

    if (sum_of_elems==0){
        _areActiveElements = false;
    }else if (sum_of_elems>0){
        _areActiveElements = true;
    }

    return _areActiveElements;
}
